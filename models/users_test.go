...
func testUserToManySessions(t *testing.T) {
	...
	slice := UserSlice{&a}
	if err = a.L.LoadSessions(tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
var (
	userDBTypes = map[string]string{`ID`: `bigint`, `AccountsID`: `character varying`, `Email`: `character varying`, `FirstName`: `character varying`, `LastName`: `character varying`, `Username`: `character varying`, `Disabled`: `boolean`, `Properties`: `jsonb`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `RemovedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
